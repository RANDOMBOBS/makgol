...
| `Public API 미세먼지 정보` <br> `공지사항 게시판` <br> `랭킹별 업장, 식당 추천` <br> `카테코리 페이지` |  <br>`Public API 날씨정보 호출` <br> `로그인, 로그아웃`<br>`쿠키활용 로그인 유지`<br>`건의사항 페이지` <br> `마이페이지` <br> `관리자용 페이지` <br> `돌림판` | `Kakao Map API` <br> `업장리스트 페이지 (거리순, 좋아요순 정렬 옵션, 간략한 업장 정보, 업장 찜하기, 업장 아이템에 마우스를 올릴시 업장 좌표로 이동) ` <br> `상세페이지 (메뉴를 포함한 상세한 업장 정보, 좋아요 기능, 본인과 업장 위치의 거리 표시, 리뷰 crud)` <br> `회원가입 로그인 아이디 비밀번호 찾기 모달` | `데이터 수집, 가공` <br> `로그인, 로그아웃` <br>`회원가입, PW찾기`<br>`JWT`<br> `Spring security` <br> `oauth2 소셜로그인` | `공룡게임`<br>`사다리게임` <br>`하소연 게시판` <br>  |
