...
- 현재 위치의 **실시간 날씨**를 기반으로 현재 날씨에 어울리는 메뉴를 추천받을 수 있습니다.
...
- 해당 식당의 상세정보 페이지로 영업시간과 해당 식당의 **메뉴 또는 가격**을 확인 가능합니다.
...
- 직장에서 겪었던 경험을 공유하는 **하소연 커뮤니티**가 존재하며 **건의사항 게시판, 공지사항이** 존재합니다.
...
| `Public API 미세먼지 정보` <br> `공지사항 게시판` <br> `랭킹별 업장, 식당 추천` <br> `카테코리 페이지` | `RestTemplate` <br>`Public API 날씨 정보 호출` <br> `건의사항 페이지` <br> `마이페이지` <br> `관리자용 페이지` <br> `돌림판` | `Kakao Map API` <br> `업장리스트 페이지` <br> `상세페이지` <br> `회원가입 로그인 스타일링` | `Public API` <br> `Kakao Map API` <br> `데이터수집, 가공` <br> `회원가입` <br>`redis`<br> `JWT` <br> `Spring security` | `사다리게임` <br>`하소연 게시판` <br>  |
|조정석|김성실|기술반장|테스트|열정맨|
